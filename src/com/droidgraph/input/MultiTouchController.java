...
//			DGScene scene) {
//		this(objectCanvas, scene, true);
//		this.scene = scene;
//	}
//
//	/** Full constructor */
//	public MultiTouchController(MultiTouchObjectCanvas<T> objectCanvas,
//			DGScene scene, boolean handleSingleTouchEvents) {
...
//					xVals[ptrId] = processingHist ? event.getHistoricalX(histIdx) : event.getX();
//					yVals[ptrId] = processingHist ? event.getHistoricalY(histIdx) : event.getY();
//					pressureVals[ptrId] = processingHist ? event.getHistoricalPressure(histIdx) : event.getPressure();
...
//						if (ptrId != actionPointer && action != MotionEvent.ACTION_MOVE) {
...
//						Shared.p("Pointer ID:(", ptrId, ")", "ActionCode:(",actions[ptrIdx], ")");
//					}
//				}
//
//				// Decode event
//				decodeTouchEvent(pointerCount, xVals,
//						yVals,
//						pressureVals,
//						pointerIds,
//						actionPointer,
...
//			float[] pressure, int[] pointerIds, int actionPointer,
//			int[] actions, int action, boolean down, long eventTime) {
//		// if (DEBUG)
...
//		mCurrPt.set(pointerCount, x, y, pressure, pointerIds, actionPointer,
//				actions, action, down, eventTime);
...
//	 */
//	public void multiTouchController() {
//		if (DEBUG)
//			Log.i("MultiTouch",
//					"Got here 6 - " + mMode + " " + mCurrPt.getNumTouchPoints()
//							+ " " + mCurrPt.isDown() + mCurrPt.isMultiTouch());
//
//		if (mCurrPt.getAction() == MotionEvent.ACTION_UP
//				|| (mCurrPt.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_POINTER_UP) {
//			// Shared.p(mCurrPt.getAction());
//			objectCanvas.handleUpEvent(mCurrPt);
//		}
//
//		switch (mMode) {
//		case MODE_NOTHING:
//			Shared.p(mCurrPt.getNumTouchPoints());
...
//				objectCanvas.updateEvents(mCurrPt);
...
//		private int[] actions = new int[MAX_TOUCH_POINTS];
...
//
//		/** Set all point info */
//		void set(int numPoints, float[] x, float[] y, float[] pressure,
//				int[] pointerIds, int actionPointer, int[] actions, int action,
...
//
//		public void updateEvents(PointInfo mCurrPt);
//
//		public void handleUpEvent(PointInfo mCurrPt);
//
//		/**
